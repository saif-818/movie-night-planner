...
export class TMDBService {
  ...
  static async discoverMovies(params: {
    ...
    try {
      const genreIds = params.genres?.map(g => TMDB_GENRES[g]).filter(Boolean).join(',');
      const excludeGenreIds = params.excludeGenres?.map(g => TMDB_GENRES[g]).filter(Boolean).join(',');
  ...
  static async getMultipleMovieDetails(movieIds: number[]): Promise<MovieDetails[]> {
    try {
  ...
  // Get poster URL
  static getPosterUrl(posterPath: string | null, size: 'small' | 'medium' | 'large' = 'medium'): string {
  ...
  static movieToText(movie: MovieDetails): string {
    ...
    return `${movie.title}. ${movie.overview} Genres: ${genreNames}. ${movie.tagline || ''}`;
  }

  // Get trending movies
  static async getTrendingMovies(timeWindow: 'day' | 'week' = 'week'): Promise<TMDBMovie[]> {
    try {
